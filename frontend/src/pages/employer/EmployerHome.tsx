...
import { Building2, LogOut, Briefcase, Users, FileText, TrendingUp } from "lucide-react";
...
const EmployerHome = () => {
  ...
  const stats = [
    { icon: Briefcase, label: "Active Jobs", value: "12", color: "text-employer" },
    { icon: Users, label: "Total Applicants", value: "348", color: "text-primary" },
    { icon: FileText, label: "Pending Reviews", value: "24", color: "text-amber-500" },
    { icon: TrendingUp, label: "This Month", value: "+18%", color: "text-jobseeker" },
  ];
      ...
      <main className="container mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="mb-6 sm:mb-8 md:mb-10 animate-fade-in">
          <div className="flex items-start sm:items-center gap-3 mb-2">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-employer-light flex items-center justify-center flex-shrink-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm sm:text-base text-muted-foreground">
                You are logged in as <span className="font-semibold text-employer">{user.role}</span>
                {user.companyRole && <span className="hidden sm:inline"> · {user.companyRole}</span>}
              </p>
            </div>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            <Button variant="employer" size="sm" className="text-xs sm:text-sm">
              <Briefcase className="w-4 h-4 sm:mr-2" />
              <span className="hidden sm:inline">Post New Job</span>
              <span className="sm:hidden">Post Job</span>
            </Button>
            <Button variant="employer-outline" size="sm" className="text-xs sm:text-sm">
